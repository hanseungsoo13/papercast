...
각 줄별로 아래와 같은 사항을 따라
첫번째줄: 논문이 해결하고자 하는 문제와 논문의 주요 기여점
두번째줄: 논문에서 제안하는 방법론과 그 방법론의 효과
세번째줄: 논문의 주요 결과와 그 결과의 의미

규칙:
"다음은 ~~ 요약 입니다." 와 같이 불필요한 문장으로 시작하지 말고 바로 요약을 시작할 것.
1,2,3 이렇게 줄 별로 번호를 붙이지 말고 가독성 있게 문장을 작성할 것.
각 줄들이 서로 자연스럽게 이어지도록 작성할 것.
"""
